...
BitcoinHard Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 6005 for connections and services them.
...
However, in open source software such as BitcoinHard Core, where everyone uses
...
pay attention to for reviewers of BitcoinHard Core code.

General BitcoinHard Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: BitcoinHard Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against BitcoinHard Core so that they can be integrated
