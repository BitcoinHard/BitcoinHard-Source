...
const char * const BITCOIN_CONF_FILENAME = "bitcoinhard.conf";
const char * const BITCOIN_PID_FILENAME = "bitcoinhardd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "bitcoinhard" is a composite category enabling all BitcoinHard-related debug output
                if(ptrCategory->count(std::string("bitcoinhard"))) {
...
#else
    const char* pszModule = "bitcoinhard";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinHard
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinHard
    // Mac: ~/Library/Application Support/BitcoinHard
    // Unix: ~/.bitcoinhard
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinHard";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinHard";
#else
    // Unix
    return pathRet / ".bitcoinhard";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty bitcoinhard.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override bitcoinhard.conf
