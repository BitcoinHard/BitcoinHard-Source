...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in bitcoinhard.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("bitcoinhard.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "bitcoinhard",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "bitcoinhard",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "bitcoinhard",               "gobject",                &gobject,                true,  {} },
    { "bitcoinhard",               "voteraw",                &voteraw,                true,  {} },
