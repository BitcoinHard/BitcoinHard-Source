...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BitcoinHard address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no bitcoinhard: URI
    if(!uri.isValid() || uri.scheme() != QString("bitcoinhard"))
        return false;
...
{
    // Convert bitcoinhard:// to bitcoinhard:
    //
    //    Cannot handle this later, because bitcoinhard:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bitcoinhard://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "bitcoinhard:");
    }
...
{
    QString ret = QString("bitcoinhard:%1").arg(info.address);
    int paramCount = 0;
...
void openConfigfile()
{
    ...
    /* Open bitcoinhard.conf with the associated application */
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinHard Core.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinHard Core (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("BitcoinHard Core (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for "BitcoinHard Core*.lnk"
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "bitcoinhardcore.desktop";
    return GetAutostartDir() / strprintf("bitcoinhardcore-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a bitcoinhardcore.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BitcoinHard Core\n";
        else
            optionFile << strprintf("Name=BitcoinHard Core (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the BitcoinHard Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add BitcoinHard Core app to startup item list
